...
- Skips re-converting files when the expected converted JPG already exists
- Deletes original HEIC files after successful conversion, including originals left over from a prior run whose JPG already exists (only when that JPG is present and non-empty)
...
- If `photo_converted.jpg` already exists, that source HEIC is treated as already converted: it is not re-converted, and the leftover `photo.heic` is deleted (only when the existing JPG is present and non-empty)
